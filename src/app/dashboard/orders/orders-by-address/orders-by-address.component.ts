...
export class OrdersByAddressComponent implements OnInit {
  ...
  groupedAddresses: { [key: string]: any[] } = {}
  ...
  groupOrders(filterKey: any) {
    this.groupedAddresses = {};
    this.address = this.totalOrders.map((val: any) => {
      return val[filterKey];
    });
    this.address = new Set(this.address)
    this.address.forEach((val: string) => {
      this.groupedAddresses[val] = this.totalOrders.filter(t => {
        if (filterKey === 'location') {
          return t['location'] === val;
        }
        else if (filterKey === 'foodItem') {
          return t['foodItem'].toString() === val.toString();
        }
        else if (filterKey === 'planType') {
          return t['planType'] === val;
        }
        else if (filterKey === 'deliverySlot') {
          return t['deliverySlot'] === val;
        }
        return;
      });
    })
    this.groupeditems = this.groupedAddresses
  }
